...
function margeArrays (arr1, arr2){
    ...
    for (let i=0; i<arr.length; i++) {
        if(occurrences[arr[i].id]){
            occurrences[arr[i].id].push(i);
            continue;
        }
        occurrences[arr[i].id] = [i];
    }

    let occurrencesKeys = Object.keys(occurrences);

    for(let i = 0; i< occurrencesKeys.length; i++){
        const occurrenceKey = occurrencesKeys[i];
        const occurrence = occurrences[occurrenceKey];
        if(occurrence.length>1){
            mergeObjects(occurrence);
        }
    }
    function mergeObjects(occurence, arr){

        return null;
    }
    //
    // let filteredFamily = arr.filter(function(x) {
    //     if (occurrences[x.id]) {
    //         return false;
    //     }
    ...
    //     // var ddd =  arr.reduce(function(sum, current) {
